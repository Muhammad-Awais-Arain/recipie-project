...
for app_id in range(117, 120):  
    # Create a folder named "weblink_responses" if it doesn't exist
    if not os.path.exists("weblink_responses"):
        os.makedirs("weblink_responses")

    # Create a folder named "weblink_differences" if it doesn't exist
    if not os.path.exists("weblink_differences"):
        os.makedirs("weblink_differences")
    ...
    api_url = f'{api_url}/rest_api/get_multiple_weblinks?app_id={app_id}&language=en'
    ...
    if response.status_code == 200:
        ...
        saved_response_filename = os.path.join("weblink_responses", f"app_{app_id}_response.json")
        if os.path.exists(saved_response_filename):
            ...
            if differences:
                ...
                # Save the new JSON response with differences to a file in "weblink_differences" folder
                diff_filename = os.path.join("weblink_differences", f"app_{app_id}_differences.json")
