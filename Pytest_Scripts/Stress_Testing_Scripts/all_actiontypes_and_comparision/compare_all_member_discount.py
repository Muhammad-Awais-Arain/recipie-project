...
for app_id in range(117, 120):  
    # Create a folder named "discount_responses" if it doesn't exist
    if not os.path.exists("discount_responses"):
        os.makedirs("discount_responses")

    # Create a folder named "discount_differences" if it doesn't exist
    if not os.path.exists("discount_differences"):
        os.makedirs("discount_differences")
    ...
    api_url = f'{api_url}/rest_api/get_member_discounts?app_id={app_id}&language=en'
    ...
    if response.status_code == 200:
        ...
        saved_response_filename = os.path.join("discount_responses", f"app_{app_id}_response.json")
        if os.path.exists(saved_response_filename):
            ...
            if differences:
                ...
                # Save the new JSON response with differences to a file in "discount_differences" folder
                diff_filename = os.path.join("discount_differences", f"app_{app_id}_differences.json")
