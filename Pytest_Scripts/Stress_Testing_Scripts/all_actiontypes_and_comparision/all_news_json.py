...
for app_id in app_id_range:
    ...
    url = f'{api_url}/rest_api/get_all_news?app_id={app_id}&language=en'
    ...
    if response.status_code == 200:
        ...
        with open(f'news_responses/app_{app_id}_response.json', 'w') as file:
            json.dump(response_data, file, indent=4)
        
        # Print a message indicating success
        print(f'Navigation(news) for App_id {app_id} is up and functioning correctly.')

    else:
        # Print a message indicating failure
        print(f'Navigation(news) for App_id {app_id} is not up and not functioning correctly.')
