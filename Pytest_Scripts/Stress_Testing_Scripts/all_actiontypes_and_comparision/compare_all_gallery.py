...
for app_id in range(117, 120):  
    # Create a folder named "gallery_responses" if it doesn't exist
    if not os.path.exists("gallery_responses"):
        os.makedirs("gallery_responses")

    # Create a folder named "gallery_differences" if it doesn't exist
    if not os.path.exists("gallery_differences"):
        os.makedirs("gallery_differences")
    ...
    api_url = f'{api_url}/rest_api/get_all_gallery?app_id={app_id}&language=en'
    ...
    if response.status_code == 200:
        ...
        saved_response_filename = os.path.join("gallery_responses", f"app_{app_id}_response.json")
        if os.path.exists(saved_response_filename):
            ...
            if differences:
                ...
                # Save the new JSON response with differences to a file in "gallery_differences" folder
                diff_filename = os.path.join("gallery_differences", f"app_{app_id}_differences.json")
