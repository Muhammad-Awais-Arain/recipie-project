...
for app_id in range(117, 120):  
    # Create a folder named "union_representative_responses" if it doesn't exist
    if not os.path.exists("union_representative_responses"):
        os.makedirs("union_representative_responses")

    # Create a folder named "union_representative_differences" if it doesn't exist
    if not os.path.exists("union_representative_differences"):
        os.makedirs("union_representative_differences")
    ...
    api_url = f'{api_url}/rest_api/get_all_union_representatives?app_id={app_id}&language=en'
    ...
    if response.status_code == 200:
        ...
        saved_response_filename = os.path.join("union_representative_responses", f"app_{app_id}_response.json")
        if os.path.exists(saved_response_filename):
            ...
            if differences:
                ...
                # Save the new JSON response with differences to a file in "union_representative_differences" folder
                diff_filename = os.path.join("union_representative_differences", f"app_{app_id}_differences.json")
