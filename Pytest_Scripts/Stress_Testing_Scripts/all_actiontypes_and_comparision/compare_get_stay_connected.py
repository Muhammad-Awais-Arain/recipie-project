...
for app_id in range(117, 120):  
    # Create a folder named "stay_connected_responses" if it doesn't exist
    if not os.path.exists("stay_connected_responses"):
        os.makedirs("stay_connected_responses")

    # Create a folder named "stay_connected_differences" if it doesn't exist
    if not os.path.exists("stay_connected_differences"):
        os.makedirs("stay_connected_differences")
    ...
    api_url = f'{api_url}/rest_api/get_stay_connected?app_id={app_id}&language=en'
    ...
    if response.status_code == 200:
        ...
        saved_response_filename = os.path.join("stay_connected_responses", f"app_{app_id}_response.json")
        if os.path.exists(saved_response_filename):
            ...
            if differences:
                ...
                # Save the new JSON response with differences to a file in "stay_connected_differences" folder
                diff_filename = os.path.join("stay_connected_differences", f"app_{app_id}_differences.json")
