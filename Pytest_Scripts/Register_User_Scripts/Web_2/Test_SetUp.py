...
website = 'https://desibook.web.linkedunion.org/'
...
def register_user():
    ...
    time.sleep(2)
    check_and_click_cancel()
    time.sleep(1)
    icon = (By.CLASS_NAME, "user-member-login-icon")
    login_icon = wait.until(EC.element_to_be_clickable(icon))
    login_icon.click()
    time.sleep(1)
...
def enter_verification_code(code):
    ...
    time.sleep(1)
    verification_code_inputs = driver.find_elements(By.NAME, 'letters[]')
    for i, digit in enumerate(code):
        verification_code_inputs[i].send_keys(digit)
    time.sleep(1)
    Verify = (By.CLASS_NAME, 'verify_register')
    Click_verify = wait.until(EC.presence_of_element_located(Verify))
    Click_verify.click()
...
def toggle_settings(twoFactor, adminApproval, unrecognized=None):
    ...
    SavePS.click()
    time.sleep(2)
    Ok = wait.until(EC.presence_of_element_located((By.XPATH, '//button[@class="swal2-confirm btn btn-sm btn-bold btn-brand"]')))
    Ok.click()
...
def login_with_email_and_password(email, password):
    ...
    time.sleep(1)
    CloseApprovalPopUp = wait.until(EC.element_to_be_clickable((By.XPATH, '//*[@id="login-modal"]/div/div/div[3]/form/div[1]/button')))
    CloseApprovalPopUp.click()

    time.sleep(1)
    login_icon = driver.find_element(By.CLASS_NAME, "user-member-login-icon")
    login_icon.click()
    time.sleep(1)

    driver.find_element(By.ID, 'generalSearch').send_keys(email)
    driver.find_element(By.NAME, 'password').send_keys(password)

    LoginBtn = wait.until(EC.presence_of_element_located((By.CLASS_NAME, "login_btn")))
    LoginBtn.click()
...
def add_member(firstName, lastName, DOB, employeeID, SSN):
    ...
    #Only for Insulators International
    # driver.find_element(By.NAME, 'industry').send_keys("Local 1F")
...
def check_and_click_cancel():
    try:
        # Wait up to 10 seconds for the element to be present
        cancel_button = wait.until(EC.presence_of_element_located((By.XPATH, '//*[@id="onesignal-slidedown-allow-button"]')))
    
        driver.execute_script("arguments[0].click();", cancel_button)
        print("Clicked on 'Cancel' button.")
    except NoSuchElementException:
        print("Element not found or not displayed. Proceeding with the code.")
...
def close_notification():
    try:
        button = wait.until(EC.presence_of_element_located((By.CLASS_NAME, 'remove_notify')))
        button.click()
    except TimeoutException:
        print("Button not found. Waiting for it to appear...")
        wait.until(EC.presence_of_element_located((By.CLASS_NAME, 'remove_notify')))
        button = driver.find_element(By.CLASS_NAME, 'remove_notify')
        button.click()
